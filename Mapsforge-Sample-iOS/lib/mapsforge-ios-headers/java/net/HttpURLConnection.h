...
/*!
 @brief An <code>URLConnection</code> for HTTP (<a href="http://tools.ietf.org/html/rfc2616">
 RFC 2616</a>) used to send and
  receive data over the web.Data may be of any type and length.
 This class may
...
@interface JavaNetHttpURLConnection : JavaNetURLConnection {
 @public
  /*!
   @brief The HTTP request method of this <code>HttpURLConnection</code>.The default
  value is <code>"GET"</code>.
   */
  NSString *method_;
  /*!
   @brief The status code of the response obtained from the HTTP request.The
  ...
  chunk-length.Default value is <code>-1</code> that means the chunked encoding
  mode is disabled.
   */
  jint chunkLength_;
  /*!
   @brief The byte count in the request body if it is both known and streamed; and
  -1 otherwise.If the byte count exceeds <code>Integer.MAX_VALUE</code> (2 GiB)
  then the value of this field will be <code>Integer.MAX_VALUE</code>.
   In that
  ...
  /*!
   @brief The byte count in the request body if it is both known and streamed; and
  -1 otherwise.Prefer this field over the <code>int</code>-valued <code>fixedContentLength</code>
...
/*!
 ...
 <code>type</code>, which may also include the
...
/*!
 @brief Returns the date value in milliseconds since <code>01.01.1970, 00:00h</code>
  corresponding to the header field <code>field</code>.The <code>defaultValue</code>
...
/*!
 @brief Stream a request body whose length is not known in advance.Old HTTP/1.0
 ...
 @param chunkLength the length to use, or <code>0</code>  for the default chunk
       length.
 @throw IllegalStateExceptionif already connected or another mode
      already set.
 */
