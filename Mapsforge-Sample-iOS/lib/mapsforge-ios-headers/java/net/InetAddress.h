...
/*!
 @brief An Internet Protocol (IP) address.This can be either an IPv4 address or an IPv6 address, and
 ...
 Most code does not need to distinguish between the two
...
/*!
 @brief Compares this <code>InetAddress</code> instance against the specified address
  in <code>obj</code>.Two addresses are equal if their address byte arrays have
...
/*!
 ...
 If the host name is empty or <code>null</code> an 
...
/*!
 @brief Equivalent to <code>getByAddress(null, ipAddress)</code>.Handy for addresses with
...
/*!
 ...
 @throw UnknownHostExceptionif <code>ipAddress</code> is null or the wrong length.
 */
...
/*!
 ...
 <code>host</code>.The host string may be either a machine name or a dotted
  string IP address.
 If the latter, the <code>hostName</code> field is
...
/*!
 @brief Returns the host name corresponding to this IP address.This may or may not be a
  fully-qualified name.
 If the IP address could not be resolved, the numeric representation
...
/*!
 @brief Tries to reach this <code>InetAddress</code>.This method first tries to use
  ICMP <i>(ICMP ECHO REQUEST)</i>, falling back to a TCP connection
  on port 7 (Echo) of the remote host.
 @param timeout timeout in milliseconds before the test fails if no connection
...
/*!
 @brief Tries to reach this <code>InetAddress</code>.This method first tries to use
  ICMP <i>(ICMP ECHO REQUEST)</i>, falling back to a TCP connection
  on port 7 (Echo) of the remote host.
 @param networkInterface the network interface on which to connection should be
              established.
