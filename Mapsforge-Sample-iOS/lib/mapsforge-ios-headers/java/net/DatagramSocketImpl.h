...
/*!
 @brief Binds the datagram socket to the given localhost/port.Sockets must be
  bound prior to attempting to send or receive data.
 @param port the port on the localhost to bind.
 @param addr the address on the multihomed localhost to bind.
...
/*!
 ...
 @param inetAddr the address of the target host which has to be connected.
 @param port the port on the target host which has to be connected.
 @throw SocketException
...
/*!
 @brief Adds this socket to the multicast group <code>addr</code>.A socket must join
  a group before being able to receive data.
 Further, a socket may be a
  member of multiple groups but may join any group only once.
 @param addr the multicast group to which this socket has to be joined.
 @throw IOException
...
/*!
 ...
 Further, a socket may be a
  member of multiple groups but may join any group only once.
 @param addr the multicast group to which this socket has to be joined.
 @param netInterface the local network interface which will receive the multicast
              datagram packets.
 @throw IOException
 if an error occurs while joining the specified multicast
              group.
...
/*!
 ...
 @param addr the multicast group to be left.
 @throw IOException
...
/*!
 @brief Removes this socket from the multicast group <code>addr</code>.
 @param addr the multicast group to be left.
 @param netInterface the local network interface on which this socket has to be
              removed.
 @throw IOException
...
/*!
 @brief Peeks at the incoming packet to this socket and returns the address of the 
 <code>sender</code>.The method will block until a packet is received or
  timeout expires.
 @param sender the origin address of a packet.
...
/*!
 @brief Receives data into the supplied datagram packet by peeking.The data is
  not removed from socket buffer and can be received again by another 
 <code>peekData()</code> or <code>receive()</code> call.
 This call blocks until
  either data has been received or, if a timeout is set, the timeout has
  been expired.
 @param pack the datagram packet used to store the data.
...
/*!
 ...
 This call will block until either data has been received or, if a timeout
  ...
  is thrown.
 @param pack the datagram packet container to fill in the received data.
 @throw IOException
...
/*!
 @brief Sends the given datagram packet <code>pack</code>.The packet contains the
  data and the address and port information of the target host as well.
 @param pack the datagram packet to be sent.
 @throw IOException
...
/*!
 @brief Sets the time-to-live (TTL) option for multicast packets sent on this
  socket.
 @param ttl the time-to-live option value. Valid values are 0 
  &lt;  ttl              &lt; = 255.
 @throw IOException
