...
/*!
 ...
 <p>
  FileChannels have a virtual pointer into the file which is referred to as a file 
 <em>position</em>. The position can be manipulated by moving it
...
/*!
 ...
 @param metadata<code>true</code>
   if the file metadata should be flushed in             addition to the file content, 
...
/*!
 ...
 @param position the starting position for the locked region.
 @param size the length of the locked region in bytes.
 @param shared a flag indicating whether an attempt should be made to acquire
              a shared lock.
...
/*!
 @brief Maps the file into memory.There can be three modes: read-only,
  read/write and private.
 After mapping, changes made to memory or the file
  channel do not affect the other storage place. 
 <p>
  Note: mapping a file into memory is usually expensive.
 @param mode one of the three mapping modes.
 @param position the starting position of the file.
 @param size the size of the region to map into memory.
...
/*!
 @brief Reads bytes from this file channel and stores them in the specified array
  ...
  actually read.
 It also increases the file position by the number of bytes read. 
 <p>
...
/*!
 ...
 <p>
  ...
  the read is completed and will then contend for the ability to read.
 @param buffers the array of byte buffers into which the bytes will be copied.
 @param start the index of the first buffer to store bytes in.
 @param number the maximum number of buffers to store bytes in.
 @return the number of bytes actually read, or -1 if the end of the file has been reached.
 @throw AsynchronousCloseException
 if this channel is closed by another thread during this read
              operation.
...
/*!
 @brief Reads up to <code>count</code> bytes from <code>src</code> and stores them in this
  channel's file starting at <code>position</code>.No bytes are transferred if 
 <code>position</code> is larger than the size of this channel's file.
 Less than 
 <code>count</code> bytes are transferred if there are less bytes
...
/*!
 ...
 <code>position</code> and writes them to <code>target</code>.No bytes are
  transferred if <code>position</code> is larger than the size of this channel's
  file.
 Less than <code>count</code> bytes are transferred if there less bytes
...
/*!
 @brief Attempts to acquire an exclusive lock on this file without blocking.The
 ...
 @param size the size of file to lock.
 @param shared true if the lock is shared.
 @return the file lock object, or <code>null</code> if the lock would overlap
          with an existing exclusive lock in another OS process.
 @throw IllegalArgumentException
...
/*!
 ...
 @param buffers the buffers containing bytes to write.
 @return the number of bytes actually written.
 @throw AsynchronousCloseException
...
/*!
 @brief Attempts to write a subset of the given bytes from the buffers to this
  file channel.This method attempts to write all <code>remaining()</code>
 ...
 @param buffers the array of byte buffers that is the source for bytes written
              to this channel.
